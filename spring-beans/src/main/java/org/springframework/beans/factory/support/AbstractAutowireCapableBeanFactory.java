...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			// using an appropriate instantiation strategy实例化对象，工厂，构造器
		...
		try {
			// 初始化Bean属性 ，第五六次调用后置处理器入口
			populateBean(beanName, mbd, instanceWrapper);
			// 第七八次调用后置处理器入口
			// 初始化Bean 执行aware接口，BeanPostProcessor.before，beanInit、init method
		...
		// 循环依赖检查，spring只能处理单例的循环依赖，其它场景需要抛出异常
		if (earlySingletonExposure) {
			// earlySingletonReference只有在检测到有循环依赖才不为空
	...
	protected Object resolveBeforeInstantiation(String beanName, RootBeanDefinition mbd) {
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					// 执行前置BeanPostProcessors
					bean = applyBeanPostProcessorsBeforeInstantiation(targetType, beanName);
					if (bean != null) {
						// 执行后置BeanPostProcessors
	...
	protected void invokeInitMethods(String beanName, Object bean, @Nullable RootBeanDefinition mbd)
		...
		// Bean 是否实现InitializingBean，实现则调用afterPropertiesSet()方法
